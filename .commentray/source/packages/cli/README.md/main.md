# `packages/cli/README.md` — commentray

<!-- commentray:block id=cli-adoption -->

## Why three install paths

The CLI supports three adoption paths because different teams have different constraints:

**`npx commentray`** is the zero-friction path. No install, no PATH setup, no permissions. `npx commentray --help` works even from a fresh checkout. This is the primary path for CI and one-off use — it removes "I don't want to install yet another tool" as a barrier.

**`npm install -g commentray`** is for teams that use Commentray daily. The global install gives you `commentray` on PATH so tab-completion and muscle memory work.
...
<!-- commentray:block id=cli-exit-codes -->
...
The `validate` command itself distinguishes errors from warnings in its output, but the exit code stays binary. This keeps shell scripts (`commentray validate && git push`) simple and predictable.
