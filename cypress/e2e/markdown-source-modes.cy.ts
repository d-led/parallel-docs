...
const SOURCE_PANE_MODE_STORAGE_KEY = "parallel-docs.codeParallelDocsStatic.sourceMarkdownPaneMode";

function expectVisibleArrows(count: number): void {
  cy.get("#parallel-docs-wide-intro-arrows .parallel-docs-wide-intro-arrow")
...
function expectArrowStartsOutsideBubble(): void {
  cy.get("#parallel-docs-wide-intro").then(($bubble) => {
    const bubbleRect = $bubble[0].getBoundingClientRect();
    cy.get("#parallel-docs-wide-intro-arrows .parallel-docs-wide-intro-arrow").each(($arrow) => {
...
function whenWideIntroVisible(callback: () => void): void {
  cy.get(shellA11y.shell).then(($shell) => {
    if ($shell.attr("data-layout") === "stretch") {
      cy.get("#parallel-docs-wide-intro").should("not.exist");
      return;
    }
    cy.get("#parallel-docs-wide-intro").should("be.visible");
    callback();
  });
}
...
function clickWideIntroNextUntilTitle(fragment: string, maxNextClicks: number): void {
  ...
  cy.get("#parallel-docs-wide-intro .parallel-docs-wide-intro-title").then(($title) => {
    if ($title.text().includes(fragment)) return;
    cy.get('#parallel-docs-wide-intro button[data-wide-intro="next"]').click();
...
function whenHomeDualLayoutWide(dualOnly: () => void): void {
  ...
  cy.get(shellA11y.shell, { timeout: 20000 }).then(($shell) => {
    ...
    return cy
      ...
      .then((doc) => {
        const intro = doc.getElementById("parallel-docs-wide-intro");
      ...
      .then(() => {
        return cy.get("body").should(($b) => {
          expect($b.find("#parallel-docs-wide-intro")).to.have.length(0);
        });
      })
      .then(() => {
...
describe("Markdown source rendering modes", () => {
  ...
  it("keeps rendered-markdown source and doc panes at the same scrollTop across block buffers", () => {
    whenHomeDualLayoutWide(() => {
       ...
       */
      cy.get("#code-pane-markdown-body #using-parallel-docs", { timeout: 20000 })
        .should("exist")
        .then(($heading) => {
      ...
      cy.get("#parallel-docs-block-readme-user-guides").should("exist");
      cy.get("#code-pane")
        .invoke("scrollTop")
        .then(() => {
          cy.get(shellA11y.docPaneBody, { timeout: 20000 }).then(($body) => {
            const docPaneBody = $body[0];
            cy.get("#parallel-docs-block-readme-user-guides").should(($anchor) => {
  ...
  it("shows wide-mode intro tour once and persists dismissal", () => {
    ...
    whenWideIntroVisible(() => {
      cy.contains("#parallel-docs-wide-intro .parallel-docs-wide-intro-title", "Welcome").should(
        "be.visible",
      );
      cy.get('#parallel-docs-wide-intro button[data-wide-intro="skip"]').click();
      cy.get("#parallel-docs-wide-intro").should("not.exist");
      cy.window()
        .its("localStorage")
      ...
      cy.get("#parallel-docs-help-tour").click();
      cy.get("#parallel-docs-wide-intro").should("be.visible");
    });
  });
  ...
  it("draws two visible intro arrows in wide mode without crossing the bubble text area", () => {
    ...
    whenWideIntroVisible(() => {
      ...
      cy.get('#parallel-docs-wide-intro button[data-wide-intro="next"]').click();
      cy.contains("#parallel-docs-wide-intro .parallel-docs-wide-intro-title", "Two views").should(
        "be.visible",
      );
      expectVisibleArrows(2);
      expectArrowStartsOutsideBubble();
  ...
  it("shows intro tour on narrow viewports with narrow-view copy", () => {
    ...
    whenWideIntroVisible(() => {
      ...
      cy.contains("#parallel-docs-wide-intro .parallel-docs-wide-intro-body", "narrow view").should(
        "be.visible",
      );
    });
  });
  ...
  it("ends with a help-button reminder and introduces share link", () => {
    ...
    whenWideIntroVisible(() => {
      ...
      cy.contains(
        "#parallel-docs-wide-intro .parallel-docs-wide-intro-title",
        "Need a refresher?",
      ).should("be.visible");
      cy.contains(
        "#parallel-docs-wide-intro .parallel-docs-wide-intro-body",
        "You can always go back to this tutorial via the help button.",
      ).should("be.visible");
      cy.get("#parallel-docs-wide-intro-arrows .parallel-docs-wide-intro-arrow").should(
        "have.length",
        1,
      );
    });
  ...
  it("shows a fallback intro action when wrap-lines toggle is hidden", () => {
    ...
    cy.get(shellA11y.shell).then(($shell) => {
      ...
      cy.get("#parallel-docs-wide-intro .parallel-docs-wide-intro-step-action")
        .should("be.visible")
