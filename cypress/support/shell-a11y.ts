...
/** Substring or pattern for `document.title` — keep aligned with `.parallel-docs.toml` `[static_site].title`. */
export const STATIC_SITE_TITLE_PATTERN = /ParallelDocs/i;
...
export const shellA11y = {
  ...
  panes: {
    ...
    parallelDocs: '[aria-label="ParallelDocs"]',
  },
  ...
  colorThemeTrigger: "#parallel-docs-theme-trigger",
  colorThemeMenu: "#parallel-docs-theme-menu",
  angleSelect: '[aria-label="ParallelDocs angle"]',
  ...
  /** ParallelDocs markdown scroll body (`code-browser.ts` `#doc-pane-body`). */
  docPaneBody: "#doc-pane-body",
  /** Wrapper class for fenced Mermaid blocks from the render pipeline. */
  parallelDocsMermaid: ".parallel-docs-mermaid",
} as const;
