...
# - Optional PARALLEL_DOCS_SCREENSHOT_CLONE_URL: shallow clone from that URL.
#
# Usage (repo root):
#   bash scripts/parallel-docs-screenshots-in-fresh-worktree.sh
#   npm run extension:vscode-readme-screenshots:desktop:fresh

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ASSETS_DST="$REPO_ROOT/.parallel-docs/source/packages/vscode/README.md/assets"
CLONE_URL="${PARALLEL_DOCS_SCREENSHOT_CLONE_URL:-}"
...
if [[ -n "$CLONE_URL" ]]; then
  ...
  copy_pngs_back "$WT/.parallel-docs/source/packages/vscode/README.md/assets"
  exit 0
fi
