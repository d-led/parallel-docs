 ...
 * - `#shell` carries `data-parallel-docs-pair-browse-href` (same-site `./browse/…/index.html`, optional legacy flat `./browse/…@….html`, or GitHub blob). Host-root `/browse/…` is forbidden so static shells stay aligned with `parallel-docs-nav-search.json` and work on GitHub Pages project URLs. Resolves without `/browse/browse/` stacking.
 * - `parallel-docs-nav-search.json` `documentedPairs[].staticBrowseUrl`, when present, uses the same `./browse/…` prefix.
 ...
 *
 * Optional live check (network): `PARALLEL_DOCS_VALIDATE_PAGES_LIVE=1` sends HEAD to the first GitHub blob URL found in the hub index.
 */
...
function assertSameSiteStaticPairBrowseUsesNavJsonDotSlashPrefix(label, href) {
  ...
  if (!href.startsWith("./browse/")) {
    fail(
      `${label}: same-site static pair browse must start with "./browse/" (same as parallel-docs-nav-search.json staticBrowseUrl), not host-root "/browse/…". Got: ${href}`,
    );
  }
}

function validateNavSearchJsonStaticBrowseUrls() {
  const p = join(siteDir, "parallel-docs-nav-search.json");
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    if (!isHubRelativeBrowseHref(t)) {
      fail(
        `parallel-docs-nav-search.json documentedPairs[${String(i)}].staticBrowseUrl: expected hub-relative browse or omit, got: ${t}`,
      );
    }
    assertSameSiteStaticPairBrowseUsesNavJsonDotSlashPrefix(
      `parallel-docs-nav-search.json documentedPairs[${String(i)}].staticBrowseUrl`,
      t,
    );
  }
}
...
async function maybeHeadGithub(url) {
  if (process.env.PARALLEL_DOCS_VALIDATE_PAGES_LIVE !== "1") return;
...
async function validateHubIndex(indexHtml) {
  ...
  const docHubHref = shellAttr(indexHtml, "data-parallel-docs-pair-browse-href");
  if (!docHubHref) {
    fail('hub index.html: missing data-parallel-docs-pair-browse-href on id="shell"');
  }
  assertDocPairHref("hub shell data-parallel-docs-pair-browse-href", docHubHref);
  assertSameSiteStaticPairBrowseUsesNavJsonDotSlashPrefix(
    "hub shell data-parallel-docs-pair-browse-href",
    docHubHref,
  );
  ...
  const pathnames = flatSlug
    ? [`/browse/${flatSlug}`, `/parallel-docs/browse/${flatSlug}`]
    : indexedInner
      ? [`/browse/${indexedInner}/index.html`, `/parallel-docs/browse/${indexedInner}/index.html`]
      : [];
...
async function validateBrowsePage(name, html) {
  ...
  const doc = shellAttr(html, "data-parallel-docs-pair-browse-href");
  if (!doc) {
    fail(`browse/${name}: missing data-parallel-docs-pair-browse-href on #shell`);
  }

  const src = firstGithubBlobHrefIn(html);
  if (src) assertGithubBlobUrl(`browse/${name} (first GitHub blob link)`, src);
  assertDocPairHref(`browse/${name} shell data-parallel-docs-pair-browse-href`, doc);
  assertSameSiteStaticPairBrowseUsesNavJsonDotSlashPrefix(
    `browse/${name} shell data-parallel-docs-pair-browse-href`,
    doc,
  );
