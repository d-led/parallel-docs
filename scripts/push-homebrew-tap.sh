#!/usr/bin/env bash
set -euo pipefail
# Clone d-led/homebrew-d-led, regenerate parallel-docs.rb from GitHub Release assets, commit, push.
...
if [ -z "${HOMEBREW_TAP_PUSH_TOKEN:-}" ]; then
  if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
    echo "HOMEBREW_TAP_PUSH_TOKEN is not set; cannot push parallel-docs.rb to d-led/homebrew-d-led." >&2
...
node scripts/generate-homebrew-formula.mjs --output "${TMP}/parallel-docs.rb"

cd "${TMP}"
git config user.name "github-actions[bot]"
git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
git add parallel-docs.rb
if git diff --staged --quiet; then
  echo "No formula changes to commit."
  exit 0
fi

git commit -m "parallel-docs ${GITHUB_REF_NAME}"
