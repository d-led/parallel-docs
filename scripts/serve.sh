#!/usr/bin/env bash
set -euo pipefail
# Builds workspace packages needed by `sidetrack serve`, then runs it.
...
# `packages/{core,render,code-sidetrack-static,cli}/src` (plus render's
# esbuild script), rebuilds on change, and restarts `sidetrack serve` so
# Node picks up new `dist/` output. You should not need to restart `serve`
# by hand: `sidetrack serve` also rebuilds `_site/` on static-site changes
...
# the process boundary). Set SIDETRACK_SERVE_NO_PACKAGE_WATCH=1
...
if [ "${SIDETRACK_SERVE_NO_PACKAGE_WATCH:-}" = "1" ]; then
