# `.sidetrack.toml` — sidetrack

<!-- sidetrack:block id=toml-lede -->

This sidetrack does not duplicate the keys already documented in [`docs/spec/storage.md`](https://github.com/d-led/sidetrack/blob/main/docs/spec/storage.md) or spelled out in comments in the real [`.sidetrack.toml`](https://github.com/d-led/sidetrack/blob/main/.sidetrack.toml) on the left—only orientation. The TOML on disk uses **`# sidetrack:start id=…` / `# sidetrack:end id=…`** around each logical section (same pairing contract as the root **README**’s `<!-- #region sidetrack:… -->` / `<!-- #endregion … -->`, but in TOML line comments so parsers stay happy).

This file on disk is the **contract** the tools read first: storage root, SCM backend, how Markdown is rendered, anchor defaults, optional Angles, and optional GitHub Pages input. Everything else in `.sidetrack/` follows paths implied here.
...
<!-- sidetrack:block id=toml-scm -->

`[scm]` — Only **`git`** is supported for **`provider`** today (see [Configuration → `[scm]`](https://github.com/d-led/sidetrack/blob/main/docs/user/config.md#scm)); core exposes a small **`ScmProvider`** abstraction, and the Git adapter supplies blob SHA and commit metadata used in staleness checks.

<!-- sidetrack:page-break -->

<!-- sidetrack:block id=toml-render -->
...
<!-- sidetrack:block id=toml-anchors -->

`[anchors]` — Default resolution order (`symbol`, `lines`, …) for tooling that resolves spans.

<!-- sidetrack:page-break -->

<!-- sidetrack:block id=toml-static-site -->

`[static_site]` — Feeds `npm run pages:build`: which source fills the code pane, which Markdown file (and optional intro) fills the sidetrack pane, toolbar GitHub links, and related file shortcuts. Normative detail: [`docs/spec/storage.md`](https://github.com/d-led/sidetrack/blob/main/docs/spec/storage.md) and the plan’s Static code browser section.
...
- Do **not** put your deployed docs URL into `[static_site].github_url`. A publish URL like `https://example.com/some_context_path/<sidetrack-root>/` is an app hosting concern, not a repo identity.
...
- If you host under a custom prefix, validate by opening both:
  - `https://example.com/some_context_path/<sidetrack-root>/`
  - `https://example.com/some_context_path/<sidetrack-root>/browse/<pair>.html`
...
<!-- sidetrack:block id=toml-angles -->

`[angles]` (commented in this repo) — After you add `{storage}/source/.default`, each primary file can own a folder of `angleId.md` files; TOML lists ids and titles and picks `default_angle`. VS Code can enable this layout and open a chosen angle; Pages still use a single `sidetrack_markdown` path until the static viewer grows a switcher.

<!-- sidetrack:page-break -->

<!-- sidetrack:block id=toml-pointers -->

**Pointers:** [`docs/user/source-region-delimiters.md`](https://github.com/d-led/sidetrack/blob/main/docs/user/source-region-delimiters.md) (delimiter table by VS Code `languageId`) · [`packages/core/src/config.ts`](https://github.com/d-led/sidetrack/blob/main/packages/core/src/config.ts) · [`packages/core/src/config.test.ts`](https://github.com/d-led/sidetrack/blob/main/packages/core/src/config.test.ts)

<!-- sidetrack:page-break -->

<!-- sidetrack:block id=toml-buffering-flow-sync -->

**BufferingFlowSynchronizer** — normative sketch (column tracing, constraints in pipeline order). On disk, [`.sidetrack.toml`](https://github.com/d-led/sidetrack/blob/main/.sidetrack.toml) keeps only the `sidetrack:start` / `sidetrack:end` anchor for this id; readable text lives here. Implementation: [`packages/core/src/buffering-flow-synchronizer.ts`](../../../packages/core/src/buffering-flow-synchronizer.ts) — fuller narrative in [that file’s sidetrack](../../../packages/core/src/buffering-flow-synchronizer.ts/main.md).
