# `scroll-sync.ts` — sidetrack

Pure functions only—no VS Code imports—so the extension stays a thin orchestrator.

**`buildBlockScrollLinks`** joins `index.json` to `<!-- sidetrack:block id=… -->` in the Markdown file. Without that join, scroll sync is only **proportional** mirroring between panes.

- Reads block ids from companion Markdown marker lines, matches `index.json` entry when `entry.sourcePath` and `sidetrackPath` align.
...
**Spec:** [`docs/spec/anchors.md`](https://github.com/d-led/sidetrack/blob/main/docs/spec/anchors.md) · [`docs/spec/blocks.md`](https://github.com/d-led/sidetrack/blob/main/docs/spec/blocks.md)
