...
Runtime stretch-row DOM synchronization is documented in `.sidetrack/source/packages/render/src/block-stretch-buffer-sync.ts/main.md` (this file emits structure).

Security: Markdown passes through `rehype-sanitize` with an explicit allow-list.

Callers: `@sidetrack/code-sidetrack-static` and root `npm run pages:build` share the HTML contract.
