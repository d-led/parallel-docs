## Plan: Add MCP Server to SideTrack ✅ COMPLETED (v0.4.0)

**TL;DR** — New `packages/mcp-server/` shared package with 16 MCP tools. CLI gets `sidetrack mcp serve` + `sidetrack mcp install`. VS Code gets `configureMcpServer` + bundled `dist/mcp-server.js`. Ships with v0.4.0.

## Architecture Overview

```
packages/mcp-server/           ← NEW shared MCP logic
  depends on: @sidetrack/core, @modelcontextprotocol/sdk, zod
  used by:  cli, vscode

packages/cli/                   ← new commands: mcp serve, mcp install
  depends on: +@sidetrack/mcp-server  (new dep, needs archunit update)
...
1. **Scaffold the package**: `package.json` (name `@sidetrack/mcp-server`, type module), `tsconfig.json` (extends base, project references to core), `tsconfig.build.json`. Dependencies: `@sidetrack/core`, `@modelcontextprotocol/sdk`, `zod`.

2. **Create `src/mcp-tools.ts`** — Pure tool definitions and handler factories. Each tool:
   - `name`: snake_case (`sidetrack_init`, `sidetrack_validate`, etc.)
   - `description`: what it does, when to use
   - `inputSchema`: JSON Schema matching CLI args/flags
   - Handler factory: `(repoRoot: string) => async (args) => { ... }` — all handlers receive the repo root, call `@sidetrack/core` APIs directly.
   ...
   | ---------------------------------- | ------------------------------------------------------------ |
   | `sidetrack_init`                   | `init`                                                       |
   | `sidetrack_validate`               | `validate [--staged]`                                        |
   | `sidetrack_paths`                  | `paths <file>`                                               |
   | `sidetrack_render`                 | `render --source --markdown --out --mermaid`                 |
   | `sidetrack_doctor`                 | `doctor [--allow-deletions]`                                 |
   | `sidetrack_migrate`                | `migrate`                                                    |
   | `sidetrack_migrate_angles`         | `migrate-angles [--angle-id] [--dry-run]`                    |
   | `sidetrack_angles_add`             | `angles add <angleId> [--source] [--title] [--make-default]` |
   | `sidetrack_sync_moved_paths`       | `sync-moved-paths [--from] [--to] [--dry-run]`               |
   | `sidetrack_convert_source_markers` | `convert-source-markers --file --language [--dry-run]`       |

3. **Create `src/mcp-server.ts`** — Exports `createMcpServer(repoRoot: string)` that returns a configured `Server` instance with all tools registered. Uses `@modelcontextprotocol/sdk`'s `McpServer`. Reports server identity: name `sidetrack-mcp`, version from its own package.json.

4. **Create `src/stdio-entry.ts`** — The actual entry point (`#!/usr/bin/env node`). Calls `findProjectRoot(process.cwd())` (reused from CLI's `project-root.ts` — extract to `@sidetrack/core` if not already exported there), creates the server, connects `StdioServerTransport`, starts. Errors go to stderr, MCP protocol on stdout.
...
5. **Create `src/mcp-install.ts`** — Install logic for repo-local MCP configs:
   ...
   - Each entry uses `sidetrack mcp serve` as the command (no absolute paths — portable across contributors)
   - `--dry-run` flag: print what would be written without touching files
   - `--force` flag: overwrite existing SideTrack entries (default: merge/skip if already present)
...
### Phase 2: CLI Integration — `sidetrack mcp serve` and `sidetrack mcp install`

7. **Add `@sidetrack/mcp-server` dependency** to `packages/cli/package.json`.

8. **Add `sidetrack mcp serve` subcommand** to `packages/cli/src/cli.ts`:

   ```
   sidetrack mcp serve
   ```

   Starts the MCP server via stdio from the current repo root. Thin wrapper that imports `stdio-entry` from `@sidetrack/mcp-server`. This is the command referenced in all generated MCP configs.

9. **Add `sidetrack mcp install` subcommand** to `packages/cli/src/cli.ts`:

   ```
   sidetrack mcp install [--dry-run] [--force]
   ```

   Calls `installMcpConfigs` from `@sidetrack/mcp-server`. Reports which files were written/updated, which harnesses were configured.

10. **Add `sidetrack mcp` parent command** with `--help` listing both subcommands.

### Phase 3: VS Code Extension Integration

11. **Add `@sidetrack/mcp-server` dependency** to `packages/vscode/package.json` (bundled by esbuild, same as core).

12. **Add `sidetrack.configureMcpServer` command** to VS Code extension:
    ...
    - Implement in `extension.ts`: shows an information message with:
      - Option A: "Run `sidetrack mcp install` in your repo" (preferred — portable, no absolute paths)
    ...
    - Also offers to run `sidetrack mcp install` directly if the CLI is available on PATH

13. **Bundle MCP server in extension dist** for standalone use:
    - Create `packages/vscode/esbuild.mcp-server.mjs` — bundles `@sidetrack/mcp-server`'s `stdio-entry` → `dist/mcp-server.js`
...
15. **Create `packages/mcp-server/src/test/mcp-tools.test.ts`** — Vitest unit tests:
    - Test each tool handler with a temp SideTrack project (init first, then exercise each tool)
...
16. **Create `packages/mcp-server/src/test/mcp-server.test.ts`** — Integration test:
    ...
    - Test `tools/call` for `sidetrack_init` and `sidetrack_validate` on a temp directory
...
18. **Create `packages/cli/src/test/mcp-cli.test.ts`** — CLI integration:
    - Test `sidetrack mcp serve --help` prints usage
    - Test `sidetrack mcp install --help` prints usage
    - Test `sidetrack mcp install --dry-run` output

19. **Create `packages/vscode/src/test/suite/mcp-extension.test.ts`** — Extension test:
    - Test `sidetrack.configureMcpServer` command shows notification with expected content
...
23. **Wire `packages/mcp-server` into workspace tooling**:
    - Add `@sidetrack/mcp-server` to `WORKSPACE_NAMES` in `scripts/sync-workspace-deps.mjs`
    - `scripts/set-workspace-versions.mjs` iterates `packages/*` — already covers the new package

24. **Add `@sidetrack/mcp-server` to publish workflow** in `scripts/publish.sh`:
    - Add to `PUBLIC_WORKSPACES` array (in dependency order: after `@sidetrack/render`, before `@sidetrack/code-sidetrack-static`, since CLI depends on it)
    - Ensure `packages/mcp-server/package.json` has `"publishConfig": { "access": "public" }` and `"files": ["dist", "LICENSE"]`

25. **Create `scripts/install-plugin-here.sh`** — Install the SideTrack VS Code extension into the currently running IDE (VS Code, Cursor, or Antigravity). Differs from `install-extension.sh` which installs into ALL detected editors — this one targets only the active IDE for faster dogfooding loops.
    - Detects the running IDE via `$VSCODE_IPC_HOOK_CLI` (VS Code/Cursor integrated terminal), `$ANTIGRAVITY_EDITOR_APP_ROOT` (Antigravity), or `$SIDETRACK_EDITOR` override
...
| `packages/cli/package.json`                            | add `@sidetrack/mcp-server` dep                                                                            |
| `packages/cli/src/cli.ts`                              | add `mcp serve`, `mcp install` commands                                                                    |
| `packages/cli/src/test/mcp-cli.test.ts`                | **NEW**                                                                                                    |
| `packages/vscode/package.json`                         | add `@sidetrack/mcp-server` dep, `configureMcpServer` command + activationEvent, `build:mcp-server` script |
...
| `scripts/publish.sh`                                   | add `@sidetrack/mcp-server` to `PUBLIC_WORKSPACES`                                                         |
| `scripts/sync-workspace-deps.mjs`                      | add `@sidetrack/mcp-server` to `WORKSPACE_NAMES`                                                           |
...
3. `sidetrack mcp serve` — starts MCP server from repo root
4. `sidetrack mcp install --dry-run` — prints planned configs, no files written
5. `sidetrack mcp install` — writes `.vscode/mcp.json`, `.claude/mcp.json`, etc. with `sidetrack mcp serve` command
6. Generated configs contain **no absolute paths** — safe to commit
7. `sidetrack.configureMcpServer` in VS Code — shows notification with config options
...
11. Manual: configure Claude Desktop with `sidetrack mcp serve` in a SideTrack repo → `sidetrack_validate` tool works
...
| **`sidetrack mcp serve` as the canonical command**         | Portable — no absolute paths. Every contributor runs the same CLI they already have. Configs are commit-safe.                      |
| **`sidetrack mcp install` writes repo-local configs only** | `.vscode/mcp.json`, `.claude/mcp.json`, etc. are project files — meant to be committed. No user-global config files touched.       |
...
| **Direct `@sidetrack/core` API calls**                     | Faster, no subprocess overhead, better error propagation.                                                                          |
