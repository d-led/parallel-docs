...
SideTrack ties three surfaces together. If they drift apart, validation fails, scroll sync misaligns, or sidetrack points at the wrong lines. This guide is the **operational contract**: what must match, how to check it, and what to do when it breaks.

## The three surfaces (and what each owns)

| Surface             | Location                                        | What you must keep aligned                                                                                                                 |
| ------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **A. Index**        | `.sidetrack/metadata/index.json`                | Per companion file: `sourcePath`, `sidetrackPath`, and each block’s `id`, `anchor`, optional `snippet` / `markerId` / verification fields. |
| **B. Markdown**     | `.sidetrack/source/…/*.md` (or per-angle paths) | For each block: a line `<!-- sidetrack:block id=<id> -->` **with the same `id` as the index**, then prose below it.                        |
...
- **Consistency:** After refactors, re-check ranges. Use **`sidetrack validate`**; fix `anchor` (and snippet if you use it) so they still describe the intended span.

### `marker:<id>` (named region in source)

- **Pros:** Tools resolve the span from **paired delimiters** in the source (`//#region sidetrack:<id>` … `//#endregion`, or `sidetrack:start id=<id>` / `sidetrack:end` where regions are not idiomatic). Renumbering lines **inside** the region does not break the link.
...
- **Consistency:** Never rename a region id in source without updating **`marker:`** / **`markerId`** and the Markdown **`id=`** and index **`id`** to the same new token. Use **`sidetrack convert-source-markers`** if you change language/comment style.
...
1. **Index** `blocks[].id` equals the Markdown `<!-- sidetrack:block id=… -->` id (same string).
2. **Index** `entry.sourcePath` and `entry.sidetrackPath` match the files you think you paired; the JSON object key must equal `sidetrackPath`.
3. **`anchor`** parses (see [anchors.md](../spec/anchors.md)).
4. If **`marker:`** anchor: source contains a well-formed pair for that id; **`sidetrack validate`** must not report marker pairing errors for that file.
5. If **`lines:`** anchor: `start`–`end` are within the file and describe the intended lines; update after line insert/delete if the sidetrack should move with different lines.
...
Run these from the **repository root** (or ensure `sidetrack` resolves paths the same way your workspace does).

| Command                                                              | Purpose                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`sidetrack init`**                                                 | Ensures dirs + `index.json`, creates `.sidetrack.toml` if missing, runs migrations/normalization, merges **`d-led.sidetrack-vscode`** into `.vscode/extensions.json` when that file is valid mergeable JSON, then **validate**. Safe to repeat.                                                                                                                   |
| **`sidetrack validate`**                                             | Schema, index keys, marker pairing, marker uniqueness across files, marker/source alignment. When a **primary file is missing**, also prints **relocation hints** (Git `HEAD~1`→`HEAD` renames, marker matches in other indexed files, and a **bounded** scan of other **Git-tracked** source files for the same heuristics). **Use in CI** (exit `1` on errors). |
| **`sidetrack doctor`**                                               | `validate` plus environment hints (e.g. missing `.git`).                                                                                                                                                                                                                                                                                                          |
| **`sidetrack migrate`**                                              | Rewrites `index.json` when schema or snippet normalization changes (also applied automatically on read in many tools).                                                                                                                                                                                                                                            |
| **`sidetrack sync-moved-paths`**                                     | After **Git renames/moves**, rewrites `sourcePath` / `sidetrackPath` in the index using `git diff` rename detection. Does not fix anchors inside files—you still need to adjust `lines:` or regions if logic moved.                                                                                                                                               |
| **`sidetrack convert-source-markers --file <path> --language <id>`** | Rewrites **source** region delimiter style to match a VS Code language id (dry-run first if unsure).                                                                                                                                                                                                                                                              |

**Editor:** “SideTrack: Validate workspace metadata” runs the same validation as the CLI and prints issues to the output channel.

**Git hook:** `sidetrack init scm` installs a **pre-commit** fragment that runs **`sidetrack validate`** when the CLI is on `PATH`. That catches index/markdown/source mistakes before they land on `main`.

## Workflows after common edits

### You moved or renamed a source or sidetrack file (Git)

1. **`sidetrack sync-moved-paths`** (optionally `--dry-run` first) to fix index paths.
2. **`sidetrack validate`** — fix any remaining path or anchor issues.
...
2. **`sidetrack convert-source-markers`** if only the comment _syntax_ changed.
3. **`sidetrack validate`**.

### You added a new block

1. Add **`<!-- sidetrack:block id=newid -->`** in the Markdown (new id must satisfy [anchors.md](../spec/anchors.md) id rules).
2. Append a **`blocks[]`** entry with the same **`id`**, correct **`anchor`**, and matching **`sourcePath`** / file key under **`bySideTrackPath`**.
3. **`sidetrack validate`**.
...
1. Remove the Markdown section (including its `<!-- sidetrack:block … -->` line).
...
2. Keep **`lines:`** but run **`sidetrack validate`** in **pre-commit** and CI so mistakes are caught immediately, **or**
3. Use the **VS Code** flow to add blocks and validate from the editor.

SideTrack does **not** silently rewrite your primary source to match stale `lines:` anchors—that is intentional. The **tenable** path is: pick an anchor strategy that matches your team’s tolerance for source markers vs line churn, then **automate validation** so inconsistency never accumulates.
...
- [Install](install.md), [Quickstart](quickstart.md), [What SideTrack detects](detection.md), [CLI reference](cli.md), [Configuration](config.md), [Troubleshooting](troubleshooting.md)
