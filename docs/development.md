...
Hands-on notes for working on ParallelDocs itself: building, debugging, and
observing the editor extension while dogfooding. If you just want to
install ParallelDocs to use it, see the top-level `README.md`.

## Dogfood: README on GitHub Pages

This repository pairs the root **`README.md`** with longer parallel-docs under **`.parallel-docs/source/README.md/`** for the **`[static_site]`** build (see the root **`.parallel-docs.toml`**). The README stays relatively factual; the parallel-docs file is the voice-over (trade-offs, cookbook, diagrams). Open the **[published site](https://d-led.github.io/parallel-docs/)** to try scroll-linked panes on the static output without installing anything.

## Layout

<!-- #region parallelDocs:dev-layout -->

- `packages/core` — shared library (`.parallel-docs.toml` parsing, path
  normalization, metadata schema, validation, git adapter).
- `packages/render` — Markdown → HTML renderer + client-side bundle for
  the static site.
- `packages/cli` — the `parallel-docs` CLI (also packaged as a standalone
  Node SEA binary). Published consumers often run **`npx parallel-docs`**; **`npx parallel-docs --help`** shows `Usage: parallel-docs [options] [command]`.
- `packages/code-parallel-docs-static` (`@parallel-docs/code-parallel-docs-static` on npm) — static-site generator for the
  rendered parallel-docs pages.
- `packages/vscode` — VS Code / Cursor extension.

<!-- #endregion parallelDocs:dev-layout -->

## Clone and workspace setup

```bash
git clone https://github.com/d-led/parallel-docs.git
cd parallel-docs
npm ci
...
<!-- #region parallelDocs:dev-quality-gate -->
...
| Variable                      | Skips                                                          |
| ----------------------------- | -------------------------------------------------------------- |
| `PARALLEL_DOCS_SKIP_VSCODE=1` | VS Code / Electron extension host (no GUI or faster iteration) |
| `PARALLEL_DOCS_SKIP_E2E=1`    | Cypress / Chrome                                               |

```bash
PARALLEL_DOCS_SKIP_VSCODE=1 npm run test:all
PARALLEL_DOCS_SKIP_E2E=1 npm run test:all
PARALLEL_DOCS_SKIP_VSCODE=1 PARALLEL_DOCS_SKIP_E2E=1 npm run test:all   # only ci:full
```
...
<!-- #endregion parallelDocs:dev-quality-gate -->

## Contributor expectations

<!-- #region parallelDocs:dev-contributor-expectations -->
...
- **Dependencies:** preview with `npm run deps:upgrade -- --check`; apply with `npm run deps:upgrade` (`patch` / `minor` / `major` / `latest`). The script re-pins `@parallel-docs/*` via `scripts/sync-workspace-deps.mjs` and refreshes the lockfile — then run `npm run quality:gate`. Triage `npm audit` seriously; avoid blanket `--force`.
- **Format:** `npm run format` (write) or `npm run format:check` (verify).
- **Coverage (discovery, not a score chase):** `npm run test:coverage` (unit) and `npm run test:coverage:all` (unit + integration) emit HTML + `lcov` under `./coverage/` (gitignored). Set `PARALLEL_DOCS_COVERAGE_OPEN=0` to skip opening a browser.
...
<!-- #endregion parallelDocs:dev-contributor-expectations -->
...
<!-- #region parallelDocs:dev-static-hub-urls -->
...
the primary file or companion Markdown: same `sourcePath` and `parallelDocsPath`
...
- **Init:** `npm run parallel-docs -- init` is idempotent (storage, seed `index.json` / `.parallel-docs.toml` when missing). Use `npm run parallel-docs -- init config` for TOML defaults, or `init config --force` to replace. `npm run parallel-docs -- init scm` refreshes the marked `pre-commit` block that runs `parallel-docs validate --staged` when the linked CLI exists at the repo root.
- **Standalone binaries / CI:** [`.github/workflows/binaries.yml`](../.github/workflows/binaries.yml); workflow artifacts expire; **`v*`** tags attach builds to [GitHub Releases](https://github.com/d-led/parallel-docs/releases). Local builds, smoke tests, and macOS quarantine: see subsections below (README **Standalone CLI binaries**).
- **Homebrew tap (binary formula):** After a tagged release is published, job **`update-homebrew-tap`** (same workflow) runs [`scripts/push-homebrew-tap.sh`](../scripts/push-homebrew-tap.sh): it clones [d-led/homebrew-d-led](https://github.com/d-led/homebrew-d-led), writes `parallel-docs.rb` via [`scripts/generate-homebrew-formula.mjs`](../scripts/generate-homebrew-formula.mjs), and pushes to **`main`** when there are changes. Set repository secret **`HOMEBREW_TAP_PUSH_TOKEN`** to a fine-grained PAT with **Contents: Read and write** on that tap only (classic PAT with `repo` scope also works). **In GitHub Actions the job fails if that secret is missing** so a tag release cannot silently omit the formula; locally, the same script exits 0 without cloning when the token is unset. The formula uses the same four release assets as the CI matrix (**darwin-arm64**, **darwin-x64**, **linux-arm64**, **linux-x64**); Windows is release-only, not Homebrew.
- **GitHub Pages:** set `[static_site]` in `.parallel-docs.toml`; `npm run pages:build` writes `_site/`. [`.github/workflows/pages.yml`](../.github/workflows/pages.yml) deploys via `workflow_run` after [`ci.yml`](../.github/workflows/ci.yml) succeeds on `main` (includes Cypress in `e2e-static`). Re-run a failed **pages** job from Actions if deploy flaked. Enable **Settings → Pages → Build: GitHub Actions**.
- **Local Pages preview (watch, dev-only):** The Node static server here is for **developers and the CLI**; **production** is still the built **`_site/`** tree on your real host (for example GitHub Pages above). **`npm run serve`** and **`npm run pages:serve`** both run [`scripts/serve.sh`](../scripts/serve.sh), which builds the CLI stack then runs [`scripts/serve-with-package-watch.mjs`](../scripts/serve-with-package-watch.mjs): that layer watches `packages/{core,render,code-parallel-docs-static,cli}/src` (and render's `esbuild-code-browser-client.mjs`), rebuilds affected workspace packages on save, and **automatically restarts** **`parallel-docs serve`** when needed so Node reloads `dist/`—you do **not** need to stop or restart the dev server by hand. Inside the CLI, **`parallel-docs serve`** watches `.parallel-docs.toml`, static-site inputs, companions under `.parallel-docs/`, and `index.json`, rebuilds **`_site/`** on change, keeps the same HTTP listener, and injects a local **browser livereload** client into generated HTML after successful rebuilds. Default port **4173** (override with `npm run serve -- --port 8080`); livereload listens on the next port when available. Set **`PARALLEL_DOCS_SERVE_NO_PACKAGE_WATCH=1`** to skip the workspace watcher (one-shot package builds only). For native file watching on the workspace tree, set **`PARALLEL_DOCS_SERVE_PACKAGE_WATCH_POLL=0`** (defaults to polling to avoid **EMFILE** with small `ulimit -n`).

<!-- #endregion parallelDocs:dev-static-hub-urls -->

### macOS quarantine (standalone CLI)

Apple’s security layer may block a downloaded `parallel-docs-darwin-*` binary until you clear the quarantine extended attribute:

```bash
xattr -d com.apple.quarantine /path/to/parallel-docs-darwin-arm64
```

Broader cleanup (all extended attributes on one file):

```bash
xattr -c /path/to/parallel-docs-darwin-arm64
```
...
From the repo root: `npm ci`, then `npm run binary:build` and `npm run binary:smoke`. If your `node` is from **Homebrew**, the SEA build may need a **nodejs.org**-style Node of the same major as CI—set **`PARALLEL_DOCS_SEA_NODE`** to that binary’s path (the build script logs what it used).
...
On push/PR, [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs job **`e2e-static`** after **`quick`**: `pages:build`, **`node scripts/validate-pages-github-links.mjs`** (GitHub blob URL shape, no doubled-browse-path stacking, optional live HEAD to the hub source URL when `PARALLEL_DOCS_VALIDATE_PAGES_LIVE=1`), Cypress in `cypress/included`, artifact **`e2e-ci-bundle`**. The static server listens on **14173** (not the dev **`parallel-docs serve`** default **4173**). After a local **`npm run pages:build`**, run **`npm run pages:validate`** (set **`PARALLEL_DOCS_VALIDATE_PAGES_LIVE=1`** to also HEAD-check `toolbar-source-github` against `github.com`). [`.github/workflows/e2e-publish-checks.yml`](../.github/workflows/e2e-publish-checks.yml) (`workflow_run` on **`ci`**) downloads that bundle and publishes JUnit to **GitHub Checks** without checking out fork PR SHAs. Ad-hoc runs: [`.github/workflows/e2e.yml`](../.github/workflows/e2e.yml) (**workflow_dispatch** only). Locally use `npm run e2e` or `npm run e2e:ci`.
...
`@parallel-docs/core`) before `vsce` packages the `.vsix`.
...
### VS Code engine compatibility (ParallelDocs extension)

<!-- #region parallelDocs:dev-vscode-engine -->
...
instead of dereferencing `undefined`. New commands should reuse the **ParallelDocs**
...
**Desktop README screenshots (automated):** `bash scripts/refresh-vscode-readme-screenshots-desktop.sh` (alias: `npm run extension:vscode-readme-screenshots:desktop`) launches that VS Code build with `--remote-debugging-port` and uses Playwright CDP (`scripts/capture-vscode-readme-screenshots-desktop.mjs`), writing several **`vscode-*.png`** files. Scenario sequence and how to extend it: **`.parallel-docs/source/packages/vscode/README.md/main.md`** (Maintainer). Optional **`PARALLEL_DOCS_VSCODE_VIEWPORT_WIDTH`** / **`PARALLEL_DOCS_VSCODE_VIEWPORT_HEIGHT`** (defaults 1200×780) and **`PARALLEL_DOCS_VSCODE_ZOOM_LEVEL`** (default 2). The temp profile hides the secondary sidebar for cleaner frames. **`bash scripts/parallel-docs-screenshots-in-fresh-worktree.sh`** runs the same in a clean worktree. Requires `npx playwright install chromium` once. Respects **`VSCODE_TEST_VERSION`** like extension tests. **Manual:** `bash scripts/refresh-vscode-readme-screenshots-manual.sh`, **`bash scripts/refresh-root-readme-screenshots.sh`** for hub README assets.
...
ParallelDocs’s declared minimum (`^1.95.0` at the time this section was added) is
...
<!-- #endregion parallelDocs:dev-vscode-engine -->

## Scroll sync and paired panes

After **ParallelDocs: Open parallel-docs beside source** (or **Add block from
selection**, which opens the pair for you), the extension wires a scroll
listener on **both** editors:

- **Source → parallelDocs:** the top visible line of the source picks a target
  line in the Markdown. If `index.json` lists `lines:` anchors that match
  `<!-- parallelDocs:block id=… -->` markers in the parallel-docs file, the
  parallel-docs scroll snaps to the block whose source range **contains** that
...
- **ParallelDocs → source:** the top visible line in the Markdown maps back to
...
### The `ParallelDocs` output channel

The extension creates an output channel named **ParallelDocs** for
user-facing reports. Today it is only written to by **ParallelDocs:
...
- Pick **ParallelDocs** from the dropdown on the right of the panel.
...
`parallel-docs init` only creates `.parallel-docs/storage` and
`.parallel-docs/metadata/index.json` — it does _not_ preseed per-file
Markdown. Those are created on demand when you:

- Open a source file and run **ParallelDocs: Open parallel-docs beside
  source**, or
- Run `parallel-docs render --source <file>` to render a file's track.

Also: `storage.dir` inside `.git/` is rejected by design (see
`SECURITY.md`). The CLI will refuse to initialize with a clear error.

### `parallel-docs` not found on PATH after `npm run cli:install`
...
Do not hand-edit `@parallel-docs/*` version pins; `scripts/sync-workspace-deps.mjs` keeps them aligned with `packages/core/package.json`.
