...
} from "@parallel-docs/render/companion-markdown-preview";
...
import {
  ...
  routePreviewHref,
} from "./parallel-docs-preview-linking.js";

export type ParallelDocsRenderedPreviewOpenArgs = {
  repoRoot: string;
  storageDir: string;
...
async function readParallelDocsIndexOrNull(repoRoot: string): Promise<ParallelDocsIndex | null> {
  try {
...
function ratioParallelDocsLineFromSourceScroll(
  codeDoc: vscode.TextDocument,
  mdDoc: vscode.TextDocument,
  range: vscode.Range,
): number {
  const codeLines = Math.max(1, codeDoc.lineCount);
  const parallelDocsLines = Math.max(1, mdDoc.lineCount);
...
function ratioSourceLine0FromParallelDocsScroll(
  codeDoc: vscode.TextDocument,
  mdDoc: vscode.TextDocument,
  topMdLine0: number,
): number {
  const parallelDocsLines = Math.max(1, mdDoc.lineCount);
  const codeLines = Math.max(1, codeDoc.lineCount);
  const fraction = topMdLine0 / Math.max(1, parallelDocsLines - 1);
...
function previewOutputUrls(
  ...
  return {
    ...
    markdownUrlBaseDirAbs: path.dirname(parallelDocsMdAbs),
    parallelDocsStorageRootAbs: path.join(repoRoot, storageDir),
  };
}
...
function webviewMainScript(nonce: string): string {
  const js = [
    ...
    "const id='parallel-docs-md-line-'+String(line);",
    ...
    "root.querySelectorAll('[id^=\"parallel-docs-md-line-\"]').forEach(function(el){",
    "var id=el.id.slice('parallel-docs-md-line-'.length);",
...
export class ParallelDocsRenderedPreviewPanel {
  private static current: ParallelDocsRenderedPreviewPanel | undefined;
  ...
  private readonly outputUrls: ParallelDocsOutputUrlOptions;
  private readonly repoRoot: string;
  private readonly sourceRelative: string;
  private readonly parallelDocsPathRel: string;
  ...
  private constructor(
    ...
    init: {
      ...
      mdDoc: vscode.TextDocument;
      htmlDirAbs: string;
      outputUrls: ParallelDocsOutputUrlOptions;
      repoRoot: string;
      sourceRelative: string;
      parallelDocsPathRel: string;
    ...
    this.parallelDocsPathRel = init.parallelDocsPathRel;
  ...
  static disposeIfOpen(): void {
    const c = ParallelDocsRenderedPreviewPanel.current;
  ...
  private handlePanelDisposed(): void {
    ...
    if (ParallelDocsRenderedPreviewPanel.current === this) {
      ParallelDocsRenderedPreviewPanel.current = undefined;
  ...
  private async refreshBlocksAndHtml(): Promise<void> {
    ...
    const index = await readParallelDocsIndexOrNull(this.repoRoot);
    this.blocks = buildBlockScrollLinks(
      index,
      this.sourceRelative,
      this.parallelDocsPathRel,
      this.mdDoc.getText(),
  ...
  private syncPreviewScrollFromSource(range: vscode.Range | undefined): void {
    ...
    const mdLine = pickParallelDocsLineForSourceDualPane(
      this.blocks,
      topSourceLine,
      this.mdDoc.lineCount,
      () => ratioParallelDocsLineFromSourceScroll(this.codeDoc, this.mdDoc, range),
    );
  ...
  private syncSourceScrollFromPreviewLine(mdLine0: number): void {
    ...
    if (!editor) return;
    const sourceLine0 =
      pickSourceLine0ForParallelDocsScroll(this.blocks, mdLine0) ??
      ratioSourceLine0FromParallelDocsScroll(this.codeDoc, this.mdDoc, mdLine0);
  ...
  private async pushHtml(): Promise<void> {
    ...
    const body = await renderParallelDocsPreviewHtml({
      markdown: this.mdDoc.getText(),
      blockScrollLinks: this.blocks,
      pipeline: { parallelDocsOutputUrls: this.outputUrls },
    });
...
<head>
  ...
  <style>
    ...
    #preview-root .parallel-docs-line-anchor { scroll-margin-top: 12px; }
...
<body>
  ...
  static async openOrReveal(args: ParallelDocsRenderedPreviewOpenArgs): Promise<void> {
    args.pauseEditorScrollSync();
    ParallelDocsRenderedPreviewPanel.disposeIfOpen();

    const parallelDocsMdAbs = args.parallelDocsUri.fsPath;
    const htmlDirAbs = path.join(args.repoRoot, args.storageDir, "_vscode-preview", "shell");
    const outputUrls = previewOutputUrls(args.repoRoot, args.storageDir, parallelDocsMdAbs);

    const mdDoc = await vscode.workspace.openTextDocument(args.parallelDocsUri);
    const index = await readParallelDocsIndexOrNull(args.repoRoot);
    ...
    const title = `ParallelDocs preview — ${path.basename(args.sourceRelative)}`;
    const panel = vscode.window.createWebviewPanel(
      "parallel-docs.renderedPreview",
      title,
    ...
    const instance = new ParallelDocsRenderedPreviewPanel(panel, {
      ...
      parallelDocsPathRel: args.parallelDocsPathRel,
      blocks,
      restoreEditorScrollSync: args.restoreEditorScrollSync,
    });
    ParallelDocsRenderedPreviewPanel.current = instance;
