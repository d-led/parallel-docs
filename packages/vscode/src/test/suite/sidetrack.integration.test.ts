/**
 * SideTrack — VS Code extension (integration)
 *
...
async function openDogfoodPairedMarkdownActiveEditor(
  ...
  await vscode.commands.executeCommand("sidetrack.openSideBySide");
...
/** Same implementation the extension calls; loaded via relative path because the EDH resolves `@sidetrack/core` package exports poorly under `require()`. */
...
function registerExtensionSurfaceTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Extension command surface", () => {
    ...
    it("Given the extension is active, when querying VS Code commands, then Angles-related SideTrack commands are registered.", async () => {
      const cmds = await vscode.commands.getCommands(true);
      assert.ok(cmds.includes("sidetrack.init"));
      assert.ok(cmds.includes("sidetrack.openSideTrackAngle"));
      assert.ok(cmds.includes("sidetrack.addAngleDefinition"));
      assert.ok(cmds.includes("sidetrack.openRenderedPreview"));
      assert.ok(cmds.includes("sidetrack.openRenderedPreviewChooseAngle"));
      assert.ok(cmds.includes("sidetrack.openCorrespondingSource"));
    });
  });
}
...
function sidetrackPairedUriForSource(
  dogfoodWorkspace: DogfoodWorkspaceAccessor,
...
function sampleSourceWithGreetRegion(includeFarewell: boolean): string {
  return [
    ...
    "  //#region sidetrack:greet",
    "  const x = `Hello, ${name}!`;",
    "  //#endregion sidetrack:greet",
    "  return x;",
    "}",
    "",
    ...(includeFarewell
...
function registerOpenPairedMarkdownTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Open paired markdown beside the source editor", () => {
    it('Given a clean workspace, when the user runs "SideTrack: Initialize workspace", then sidetrack.openSideBySide can create the paired markdown.', async () => {
      ...
      assert.ok(text.includes("# SideTrack"));
    });

    it('Given a workspace with SideTrack configured, when the user runs "SideTrack: Open paired markdown beside editor" from a source file, then the paired Markdown file exists under `.sidetrack/source/` and starts with a SideTrack heading.', async () => {
      await vscode.commands.executeCommand("sidetrack.init");

      const text = await openSideBySideAndReadPairedText(dogfoodWorkspace);
      assert.ok(
        text.includes("# SideTrack"),
    ...
    it("Given a primary file URI (as from Explorer), when the user runs open paired with that URI, then the paired Markdown is created the same way.", async () => {
      ...
      await vscode.commands.executeCommand("sidetrack.openSideBySide", sampleUri);
      ...
      assert.ok(text.includes("# SideTrack"), "Expected paired file from URI-driven open.");
    });
  });
}
...
function registerValidateWorkspaceTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Validate workspace", () => {
    ...
    it("Given a clean dogfood tree (no generated `.sidetrack/` yet), when `validateProject` runs on that folder, then it reports storage or index warnings — not a config load failure.", async () => {
      ...
      assert.ok(
        !text.includes("Failed to load .sidetrack.toml"),
        "Dogfood .sidetrack.toml must remain valid for this fixture.",
      );
    });

    it('Given an open folder, when the user runs "SideTrack: Validate workspace", then the command completes and core `validateProject` still reports the same issues afterward.', async () => {
      const root = dogfoodWorkspace.root().fsPath;
      const before = await validateProject(root);
      await vscode.commands.executeCommand("sidetrack.validateWorkspace");
    ...
    it("Given paired Markdown was opened once, when `validateProject` runs, then the `.sidetrack/source` tree exists so validation no longer warns about a missing source directory.", async () => {
      await vscode.commands.executeCommand("sidetrack.init");
      await openFixtureSourceFile(dogfoodWorkspace.root());
      await vscode.commands.executeCommand("sidetrack.openSideBySide");
      const root = dogfoodWorkspace.root().fsPath;
      ...
      const text = issues.map((i) => i.message).join("\n");
      assert.ok(
        !text.includes("Missing directory: .sidetrack/source"),
...
async function assertTypescriptSelectionBlockFlow(
  ...
  await vscode.commands.executeCommand("sidetrack.startBlockFromSelection");
  ...
  assert.ok(
    sampleText.includes("//#region sidetrack:") && sampleText.includes("//#endregion sidetrack:"),
    "Expected TypeScript SideTrack region delimiters around the selection in the primary file.",
  );

  const mdBytes = await vscode.workspace.fs.readFile(pairedUri);
  const mdText = new TextDecoder("utf-8").decode(mdBytes);
  assert.ok(
    mdText.includes("<!-- sidetrack:block id="),
...
async function assertMarkdownSelectionBlockFlow(
  ...
  assert.ok(
    mutated.includes("<!-- #region sidetrack:"),
    "Expected Markdown start region marker in primary source.",
  );
  assert.ok(
    mutated.includes("<!-- #endregion sidetrack:"),
  ...
  const markerMatch = /<!--\s*sidetrack:block\s+id=([a-z0-9][a-z0-9-]{0,63})\s*-->/i.exec(
    pairedText,
  );
...
async function assertManualCompanionContentIsPreservedOnAddBlock(
  ...
  assert.ok(
    next.includes("<!-- sidetrack:block id=existing -->"),
...
async function assertSelectionInsideExistingRegionRevealsExistingBlock(
  ...
  const pairedUri = sidetrackPairedUriForSource(dogfoodWorkspace, "src", "existing-region.ts");
  const pairedBefore = [
    "<!-- sidetrack:block id=greet -->",
    "## greet",
    "",
    "already documented",
    "",
...
async function assertSelectionTouchingBoundaryRecoversMissingBlock(
  ...
  const pairedUri = sidetrackPairedUriForSource(dogfoodWorkspace, "src", "boundary-recovery.ts");
  ...
  assert.ok(
    pairedAfter.includes("<!-- sidetrack:block id=greet -->"),
...
async function assertStaleIndexMarkerIdCollisionIsHandled(
  ...
  const index = JSON.parse(indexRaw) as {
    ...
    bySideTrackPath: Record<
      string,
      { sourcePath: string; sidetrackPath: string; blocks: Array<Record<string, unknown>> }
    >;
  };
  const entry = index.bySideTrackPath[pairedMarkdownPath];
  index.bySideTrackPath[pairedMarkdownPath] = {
    ...(entry ? { ...entry } : {}),
    sourcePath: "src/sample.ts",
    sidetrackPath: pairedMarkdownPath,
  ...
  const markerMatch = /sidetrack:([a-z0-9][a-z0-9_-]{0,62}[a-z0-9])/i.exec(sourceAfter);
...
async function assertUnsortedInsertionsReorderedBySourceFlow(
  ...
  const sourceContent = [
    ...
    "",
    "export function farewell(name: string): string {",
    "  //#region sidetrack:farewell",
    "  return `Goodbye, ${name}.`;",
    "  //#endregion sidetrack:farewell",
    "}",
    "",
  ].join("\n");
  await replaceDocumentText(sourceUri, sourceContent);
  ...
  const pairedUri = sidetrackPairedUriForSource(dogfoodWorkspace, "src", "unsorted-order.ts");
  const pairedSeed = [
    "# custom",
    "",
    "<!-- sidetrack:block id=farewell -->",
    "## farewell",
    "",
    "bottom block",
    "",
  ...
  const markdownOrder = async (): Promise<string[]> => {
    ...
    return import("../../../../core/dist/marker-validation.js").then((m) =>
      m.extractSideTrackBlockIdsInMarkdownOrder(text),
    );
  };
...
function registerAddBlockFromSelectionTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Add side-track block from selection", () => {
    it('Given a source file with a non-empty selection, when the user runs "SideTrack: Add side-track block from selection", then the paired Markdown gains a `sidetrack:block` marker and the metadata index is updated.', async function () {
      this.timeout(90_000);
...
function registerActiveEditorUiFlagsContractTests(
  ...
  describe("Active editor UI flags (core contract for menus)", () => {
    it("Given dogfood paths, when core computes UI flags, then the companion markdown is under the tree and resolvable and the primary file is not", async () => {
      ...
      const root = dogfoodWorkspace.root().fsPath;
      const companion = sidetrackActiveEditorUiFlags({
      ...
      const primary = sidetrackActiveEditorUiFlags({
...
function registerOpenCorrespondingSourceTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Open corresponding source file from companion markdown", () => {
    it('Given the paired SideTrack markdown is active, when the user runs "SideTrack: Open corresponding source file", then the primary source editor becomes active for that pair.', async () => {
...
function registerMarkdownPreviewTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Open Markdown preview for paired file", () => {
    it('Given the paired Markdown file is active, when the user runs "SideTrack: Open Markdown preview for paired file", then the command runs without rejecting.', async () => {
      await openDogfoodPairedMarkdownActiveEditor(dogfoodWorkspace);

      await vscode.commands.executeCommand("sidetrack.openSideTrackPreview");
    });

    it('Given a primary source file that is not Markdown is active, when the user runs "SideTrack: Open Markdown preview for paired file", then the command runs without rejecting.', async () => {
      await openFixtureSourceFile(dogfoodWorkspace.root());
      await vscode.commands.executeCommand("sidetrack.openSideTrackPreview");
    });
  });
...
function registerRenderedPreviewTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Open rendered SideTrack preview (library HTML + webview)", () => {
    it('Given a primary source file is active, when the user runs "SideTrack: Open rendered SideTrack preview (default angle)", then the command completes without rejecting.', async () => {
      const editor = await openFixtureSourceFile(dogfoodWorkspace.root());
      await vscode.window.showTextDocument(editor.document, { preview: false });
      await vscode.commands.executeCommand("sidetrack.openRenderedPreview");
    });
  });
}
...
function registerSelfHealingTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("Self-healing region markers (repair command)", () => {
    it("Given a source file with missing sidetrack region markers, when repair file command is executed, then the missing markers are restored.", async function () {
      this.timeout(90_000);
      await vscode.commands.executeCommand("sidetrack.init");
      ...
      const match = /sidetrack:([a-z0-9][a-z0-9_-]{0,62}[a-z0-9])/i.exec(docText);
      ...
      await vscode.commands.executeCommand("sidetrack.repairFile");

      const healedText = activeEd.document.getText();
      assert.ok(
        healedText.includes(`//#region sidetrack:${blockId}`),
        `Expected start region marker for ${blockId} to be restored`,
      );
      assert.ok(
        healedText.includes(`//#endregion sidetrack:${blockId}`),
...
function registerRenameWatcherTests(dogfoodWorkspace: DogfoodWorkspaceAccessor): void {
  describe("SCM rename watcher", () => {
    it("Given a primary source file is renamed, when the rename event is processed, then the companion Markdown is renamed on disk and the index.json is updated.", async function () {
      ...
      const oldCompanionUri = sidetrackPairedUriForSource(
        dogfoodWorkspace,
        "src",
        "rename-temp.ts",
      );
      const newCompanionUri = sidetrackPairedUriForSource(
        dogfoodWorkspace,
        "src",
        "rename-temp-new.ts",
      );
...
describe("SideTrack in VS Code (integration)", () => {
