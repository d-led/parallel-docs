...
export const pairedMarkdownPath = ".sidetrack/source/src/sample.ts/main.md";
...
const DOGFOOD_SAMPLE_TS_BYTES = new TextEncoder().encode(
  [
    "// Sample source file for exercising the SideTrack VS Code extension.",
    "//",
    "// Open this file and run **SideTrack: Open paired markdown beside editor** (or",
    "// the angle / rendered-preview commands) to work with the companion Markdown",
    "// under `.sidetrack/source/src/sample.ts/`. README screenshots use longer",
...
/**
 * Registers suite-level `before` / `beforeEach` that resolve the dogfood folder and reset `.sidetrack/`.
 */
export function registerDogfoodWorkspaceLifecycle(): DogfoodWorkspaceAccessor {
  ...
  beforeEach(async () => {
    ...
    await resetGeneratedSideTrackStorage(workspaceRoot);
...
/** Bytes of `fixtures/dogfood/.sidetrack.toml` (restored after Angles tests mutate it). */
const DOGFOOD_SIDETRACK_TOML_BYTES = new TextEncoder().encode(
  `# Fixture SideTrack config for the VS Code extension dogfood folder.
...
export async function restoreDogfoodSideTrackToml(workspaceRoot: vscode.Uri): Promise<void> {
  const tomlUri = vscode.Uri.joinPath(workspaceRoot, ".sidetrack.toml");
  await vscode.workspace.fs.writeFile(tomlUri, DOGFOOD_SIDETRACK_TOML_BYTES);
}

/** Enables Angles without importing `@sidetrack/core` (extension host resolves CJS vs package exports poorly). */
export async function enableAnglesDogfoodFixture(workspaceRoot: vscode.Uri): Promise<void> {
  const enc = new TextEncoder();
  const sentinelUri = vscode.Uri.joinPath(workspaceRoot, ".sidetrack", "source", ".default");
  await vscode.workspace.fs.writeFile(
    sentinelUri,
    enc.encode("# SideTrack Angles layout sentinel (fixture).\n"),
  );
...
export async function resetGeneratedSideTrackStorage(workspaceRoot: vscode.Uri): Promise<void> {
  const sidetrackDir = vscode.Uri.joinPath(workspaceRoot, ".sidetrack");
  try {
    await vscode.workspace.fs.delete(sidetrackDir, { recursive: true, useTrash: false });
  } catch {
