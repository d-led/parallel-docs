...
async function withSideTrackAnglesSentinelRepo(
  tmpPrefix: string,
  fn: (repoRoot: string) => Promise<void>,
): Promise<void> {
  const tmpRepo = await mkdtemp(path.join(os.tmpdir(), tmpPrefix));
  try {
    const sentinel = path.join(tmpRepo, ".sidetrack", "source", ".default");
...
describe("sidetrackStorageSourcePrefix", () => {
  it("given a storage dir with backslashes, when building the prefix, then the prefix uses forward slashes", () => {
    expect(sidetrackStorageSourcePrefix(".sidetrack")).toBe(".sidetrack/source/");
    expect(sidetrackStorageSourcePrefix("docs-cr")).toBe("docs-cr/source/");
  });
});

describe("sidetrackActiveEditorUiFlags", () => {
  ...
  it("given a flat companion markdown path and angles off, when computing flags, then both companion flags are set", () => {
    const flags = sidetrackActiveEditorUiFlags({
      normalizedRepoRelativePath: ".sidetrack/source/src/sample.ts.md",
      storageDir: ".sidetrack",
  ...
  it("given a path under storage that is not companion markdown, when computing flags, then under-tree is true but markdown is not resolvable", () => {
    const flags = sidetrackActiveEditorUiFlags({
      normalizedRepoRelativePath: ".sidetrack/source/README.txt",
      storageDir: ".sidetrack",
      repoRoot: "/tmp/ignored",
    });
    expect(flags).toEqual({
  ...
  it("given angles sentinel exists on disk, when the path is a per-angle companion markdown, then markdown is resolvable", async () => {
    await withSideTrackAnglesSentinelRepo("cr-ui-angles-", async (tmpRepo) => {
      const flags = sidetrackActiveEditorUiFlags({
        normalizedRepoRelativePath: ".sidetrack/source/pkg/mod.ts/main.md",
        storageDir: ".sidetrack",
        repoRoot: tmpRepo,
      });
      expect(flags).toEqual({
  ...
  it("given angles sentinel exists, when the companion uses another angle id, then markdown is still resolvable", async () => {
    await withSideTrackAnglesSentinelRepo("cr-ui-angles-readme-", async (tmpRepo) => {
      const flags = sidetrackActiveEditorUiFlags({
        normalizedRepoRelativePath: ".sidetrack/source/README.md/architecture.md",
        storageDir: ".sidetrack",
        repoRoot: tmpRepo,
      });
      expect(flags).toEqual({
  ...
  it("given static_site.sidetrack_markdown points outside storage, when that file is active, then it is still treated as a resolvable companion", () => {
    const flags = sidetrackActiveEditorUiFlags({
      normalizedRepoRelativePath: "README.md.md",
      storageDir: ".sidetrack",
      repoRoot: "/tmp/ignored",
      staticSiteSideTrackMarkdownFile: "README.md.md",
    });
    expect(flags).toEqual({
      underCompanionSourceTree: true,
      isResolvableCompanionMarkdown: true,
