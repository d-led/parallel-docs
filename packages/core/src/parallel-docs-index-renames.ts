...
/**
 * When Angles layout is active, returns the angle id segment from a parallel-docs path under
 * `{storage}/source/<sourcePath>/<angle>.md`, or null if the path does not match that shape.
 */
export function inferAngleIdFromParallelDocsPath(
  ...
  if (!parallelDocsPath.startsWith(prefix)) return null;
  const rest = parallelDocsPath.slice(prefix.length);
 ...
 * - exact renames on `sourcePath` and `parallelDocsPath` (string equality),
 * - when `sourcePath` changes, recomputes `parallelDocsPath` from layout rules so companion paths stay paired.
 *
 * Renames should be sorted longest-`from` first by the caller; this function sorts defensively.
 */
export function applyPathRenamesToParallelDocsIndex(
  index: ParallelDocsIndex,
  renames: readonly ScmPathRename[],
  repoRoot: string,
  config: ResolvedParallelDocsConfig,
): { index: ParallelDocsIndex; changed: boolean } {
  ...
  for (const [, entry] of Object.entries(index.byParallelDocsPath)) {
    const sp = applyExactPathRenames(entry.sourcePath, sorted);
    let cp = applyExactPathRenames(entry.parallelDocsPath, sorted);
    ...
    if (sp !== entry.sourcePath) {
      if (anglesLayout) {
        ...
        if (angleId) {
          try {
            cp = parallelDocsMarkdownPathForAngle(
              sp,
        ...
        cp = parallelDocsMarkdownPath(sp, config.storageDir);
      }
    }
    ...
    if (sp !== entry.sourcePath || cp !== entry.parallelDocsPath) {
      changed = true;
    }

    if (next[cp]) {
      throw new Error(
        `After applying renames, two index entries map to the same parallelDocsPath "${cp}" ` +
  ...
  return {
    index: { schemaVersion: index.schemaVersion, byParallelDocsPath: next },
    changed,
  };
}
