import type { SideTrackStaticBrowsePairPaths } from "./browse-contract.js";
 ...
 * structure as on disk (e.g. `.sidetrack/source/.sidetrack.toml/main.md` →
 * `browse/.sidetrack.toml/main/index.html`).
 */
export function staticBrowseIndexRelPathFromPair(
  pair: SideTrackStaticBrowsePairPaths,
  storageDir: string,
): string {
  const norm = normalizeRepoRelativePath(pair.sidetrackPath.replaceAll("\\", "/"));
