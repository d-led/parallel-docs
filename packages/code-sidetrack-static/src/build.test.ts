...
import type { SideTrackStaticAssetCopy } from "@sidetrack/render";
import { afterEach, describe, expect, it } from "vitest";
import { buildSideTrackStatic } from "./build.js";
...
describe("Static browse HTML build — shell", () => {
  ...
  it("should write HTML that embeds highlighted source and the companion pane", async () => {
    ...
    expect(html).toContain(
      '<meta name="description" content="Test — Side-by-side source and sidetrack documentation." />',
    );
    expect(html).toContain('<meta name="generator" content="SideTrack @sidetrack/render@');
    expect(html).toContain("@sidetrack/code-sidetrack-static@");
  ...
  it("should show the provided repo-relative path in the nav rail context", async () => {
    ...
    await buildSideTrackStatic({
      ...
      markdownFile: path.join(pkgRoot, "fixtures", "sample.md"),
      outHtml,
      filePath: "packages/code-sidetrack-static/fixtures/sample.ts",
    });
    const html = await readFile(outHtml, "utf8");
    expect(html).toContain('aria-label="Current documentation pair"');
    expect(html).toContain("packages/code-sidetrack-static/fixtures/sample.ts");
  });
...
describe("Static browse HTML build — URLs and toolbar", () => {
  ...
  it("should rewrite companion links using sidetrackOutputUrls", async () => {
    outDir = await mkdtemp(path.join(tmpdir(), "ccrs-"));
  ...
  it("should copy mirrored companion images into _site/sidetrack-static-assets", async () => {
    outDir = await mkdtemp(path.join(tmpdir(), "ccrs-mirror-"));
    const repoRoot = path.join(outDir, "repo");
    const storage = path.join(repoRoot, ".sidetrack");
    ...
    await buildSideTrackStatic({
      ...
      sidetrackOutputUrls: {
        ...
        sidetrackStorageRootAbs: storage,
        staticSiteOutDirAbs: path.join(repoRoot, "_site"),
        companionStaticAssetCopies,
      },
    });
    const html = await readFile(outHtml, "utf8");
    expect(html).toMatch(/sidetrack-static-assets\/source\/pkg\/pic\.svg/);
