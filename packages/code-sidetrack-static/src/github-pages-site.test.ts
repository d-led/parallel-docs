...
async function writeMinimalPagesFixture(
  ...
  const sidetrackMarkdown = opts.sidetrackMarkdown ?? ".sidetrack/source/src/x.ts.md";
  if (sidetrackMarkdown.trim().length > 0) {
    staticSite.push(`sidetrack_markdown = "${sidetrackMarkdown}"`);
  }
  ...
  await mkdir(path.join(repo, ".sidetrack", "source", "src"), { recursive: true });
  await writeFile(path.join(repo, ".sidetrack", "source", "src", "x.ts.md"), "# Doc\n", "utf8");
}
...
async function writeReadmeAnglesCompanionStubs(repo: string): Promise<void> {
  ...
  await mkdir(path.join(repo, ".sidetrack", "source", "README.md"), { recursive: true });
  await writeFile(
    path.join(repo, ".sidetrack", "source", "README.md", "main.md"),
    "# Main angle\n",
    "utf8",
  );
  await writeFile(
    path.join(repo, ".sidetrack", "source", "README.md", "architecture.md"),
    "# Architecture angle\n",
    "utf8",
  );
}
...
async function seedReadmeAnglesPagesFixture(repo: string, title: string): Promise<void> {
  await ensureAnglesSentinelFile(repo, ".sidetrack");
  await writeReadmeMultiAngleHubToml(repo, title);
  await writeReadmeAnglesCompanionStubs(repo);
}

const README_MAIN_ANGLE_MD = ".sidetrack/source/README.md/main.md";
const EXTRA_TS_MAIN_MD = ".sidetrack/source/extra.ts/main.md";

/** `#shell` opening tag only (matches validate-pages-github-links.mjs). */
function dataSideTrackPairBrowseHrefFromShellTag(html: string): string | null {
  const m = /<div\b[^>]*\bid="shell"(?=\s|>)[^>]*>/.exec(html);
  if (!m) return null;
  const am = /\bdata-sidetrack-pair-browse-href="([^"]*)"/.exec(m[0]);
  return am?.[1] ?? null;
}
...
async function expectBrowseShellPairHrefMatchesNavStaticBrowseUrlForEveryPair(
  ...
  const navPath = path.join(siteRoot, "sidetrack-nav-search.json");
  ...
  for (const p of nav.documentedPairs ?? []) {
    ...
    expect(dataSideTrackPairBrowseHrefFromShellTag(html)).toBe(u);
  }
}
...
async function seedAnglesHubWithIndexedExtraPair(input: {
  ...
  await mkdir(path.join(repo, ".sidetrack", "source", "extra.ts"), { recursive: true });
  await writeFile(
  ...
  if (input.withDiskOnlyOrphan) {
    ...
    await mkdir(path.join(repo, ".sidetrack", "source", "orphan.ts"), { recursive: true });
    orphanCr = ".sidetrack/source/orphan.ts/main.md";
    await writeFile(
      path.join(repo, orphanCr),
      "<!-- sidetrack:block id=o1 -->\n\n## Orphan doc\n",
      "utf8",
    );
  }
  ...
  await writeIndex(repo, {
    ...
    bySideTrackPath: {
      [README_MAIN_ANGLE_MD]: {
        ...
        sidetrackPath: README_MAIN_ANGLE_MD,
        blocks: [],
      },
      [EXTRA_TS_MAIN_MD]: {
        sourcePath: "extra.ts",
        sidetrackPath: EXTRA_TS_MAIN_MD,
...
async function runWritesSiteAndNavFromFlatCompanions(): Promise<string> {
  ...
  expect(browseHtml).toMatch(/id="shell"[^>]*data-sidetrack-pair-browse-href="\.\/browse\/[^"]+"/);
  return r;
}
...
async function runAngleSelectorOnBrowsePermalinks(): Promise<string> {
  ...
  for (const angleId of ["main", "architecture"] as const) {
    ...
    expect(browseHtml).toContain('aria-label="SideTrack angle"');
    expect(browseHtml).toContain('id="sidetrack-multi-angle-b64"');
    expect(browseHtml).not.toContain("Redirecting…");
    expect(browseHtml).toContain('href="../../../index.html"');
    expect(browseHtml).toContain('aria-label="Documentation home"');
  }
...
async function runGithubToolbarUsesBlobUrlsForRealisticHost(): Promise<string> {
  ...
  const { outHtml, navSearchPath } = await buildGithubPagesStaticSite({ repoRoot: r });
  const html = await readFile(outHtml, "utf8");
  /** Hub `index.html` prefers same-site pair browse; GitHub blobs live in `sidetrack-nav-search.json`. */
  expect(html).toMatch(/data-sidetrack-pair-browse-href="\.\/browse\/[^"]+"/);
  expect(html).not.toContain("/browse/browse/");
  const nav = JSON.parse(await readFile(navSearchPath, "utf8")) as {
    documentedPairs?: { sourceOnGithub?: string; sidetrackOnGithub?: string }[];
  };
  expect(nav.documentedPairs?.[0]?.sourceOnGithub).toBe(
    "https://github.com/d-led/sidetrack/blob/main/src/x.ts",
  );
  expect(nav.documentedPairs?.[0]?.sidetrackOnGithub).toBe(
    "https://github.com/d-led/sidetrack/blob/main/.sidetrack/source/src/x.ts.md",
  );
  const browseHtml = await readFile(
    path.join(r, "_site", "browse", "src", "x.ts", "index.html"),
    "utf8",
  );
  expect(browseHtml).toMatch(/data-sidetrack-pair-browse-href="\.\/browse\/[^"]+"/);
...
async function runBrowseSingleAnglePairEmbedsScrollBlockLinks(): Promise<string> {
  ...
  const nav = JSON.parse(
    await readFile(path.join(r, "_site", "sidetrack-nav-search.json"), "utf8"),
  ) as {
  ...
  const extraRel = staticBrowseIndexRelPathFromPair(
    { sourcePath: "extra.ts", sidetrackPath: extraCr },
    ".sidetrack",
  );
  const browseHtml = await readFile(
...
async function runBrowseIndexedPairGetsScrollLinksDiskOnlyPairGetsEmptyPayload(): Promise<string> {
  ...
  const orphanRel = staticBrowseIndexRelPathFromPair(
    { sourcePath: "orphan.ts", sidetrackPath: orphanCr },
    ".sidetrack",
  );
...
describe("GitHub Pages static site output — hub, angles, and defaults", () => {
  ...
  it("uses resolved storage companion path when static_site.sidetrack_markdown is unset", async () => {
    ...
    expect(html).toContain('data-search-sidetrack-path=".sidetrack/source/src/x.ts.md"');
    expect(html).toContain('title=".sidetrack/source/src/x.ts.md">.sidetrack/source/src/x.ts.md<');
  });
...
describe("GitHub Pages static site output — nav pairs and toolbar", () => {
  ...
  it("does not set staticBrowseUrl when the indexed companion path is missing on disk (avoids hub 404s)", async () => {
    ...
    await writeIndex(repo, {
      schemaVersion: CURRENT_SCHEMA_VERSION,
      bySideTrackPath: {
        ".sidetrack/source/src/x.ts.md": {
          sourcePath: "src/x.ts",
          sidetrackPath: ".sidetrack/source/src/x.ts.md",
          blocks: [],
        },
        ".sidetrack/source/src/missing.md": {
          sourcePath: "src/x.ts",
          sidetrackPath: ".sidetrack/source/src/missing.md",
          blocks: [],
        },
      },
    });
    ...
    const nav = JSON.parse(
      await readFile(path.join(repo, "_site", "sidetrack-nav-search.json"), "utf8"),
    ) as {
      documentedPairs?: Array<{ sidetrackPath: string; staticBrowseUrl?: string }>;
    };
    const real = nav.documentedPairs?.find(
      (p) => p.sidetrackPath === ".sidetrack/source/src/x.ts.md",
    );
    const ghost = nav.documentedPairs?.find(
      (p) => p.sidetrackPath === ".sidetrack/source/src/missing.md",
    );
    expect(real?.staticBrowseUrl).toBe("./browse/src/x.ts/index.html");
    expect(ghost?.staticBrowseUrl).toBeUndefined();
  });

  it("writes GitHub blob toolbar URLs for d-led/sidetrack + main when configured", async () => {
...
describe("GitHub Pages static site — pair-browse shell matches nav JSON (regression)", () => {
  ...
  it("sets each browse #shell data-sidetrack-pair-browse-href to that pair's staticBrowseUrl", async () => {
  ...
  it("sets the hub #shell pair-browse href to the same staticBrowseUrl string as in sidetrack-nav-search.json", async () => {
    ...
    expect(dataSideTrackPairBrowseHrefFromShellTag(hubHtml)).toBe(expected);
  });
});
...
describe("GitHub Pages static site output — assets and rendered hub", () => {
  ...
  it("mirrors companion storage images under _site/sidetrack-static-assets for Pages-style hosts", async () => {
    repo = await mkdtemp(path.join(tmpdir(), "cr-pages-img-"));
    await writeMinimalPagesFixture(repo, { title: "Img" });
    await mkdir(path.join(repo, ".sidetrack", "source", "src", "assets"), { recursive: true });
    await writeFile(
      path.join(repo, ".sidetrack", "source", "src", "assets", "x.svg"),
      "<svg/>",
      "utf8",
    );
    await writeFile(
      path.join(repo, ".sidetrack", "source", "src", "x.ts.md"),
      "# Doc\n\n![](assets/x.svg)\n",
      "utf8",
    );
    await buildGithubPagesStaticSite({ repoRoot: repo });
    ...
    expect(indexHtml).toMatch(/sidetrack-static-assets\/source\/src\/assets\/x\.svg/);
  });
  ...
  it("rewrites rendered source-markdown local links to GitHub blob URLs when enabled", async () => {
    ...
    await writeFile(path.join(repo, ".sidetrack", "source", "README.md.md"), "# Doc\n", "utf8");
    await buildGithubPagesStaticSite({ repoRoot: repo });
    const html = await readFile(path.join(repo, "_site", "index.html"), "utf8");
