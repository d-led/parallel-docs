...
describe("pageBreakHostsBetweenAnchors", () => {
  ...
  it("lists page breaks only between the block anchor and the next block anchor", () => {
    ...
    b1.id = "parallel-docs-block-a";
    const pb = document.createElement("div");
    pb.className = "parallel-docs-page-break";
    pb.setAttribute("data-parallel-docs-page-break", "true");
    const b2 = document.createElement("div");
    b2.id = "parallel-docs-block-b";
    const outerPb = document.createElement("div");
    outerPb.className = "parallel-docs-page-break";
    outerPb.setAttribute("data-parallel-docs-page-break", "true");
  ...
  it("returns all page breaks after the anchor when the upper bound is null", () => {
    ...
    pb0.className = "parallel-docs-page-break";
    pb0.setAttribute("data-parallel-docs-page-break", "true");
    const pb1 = document.createElement("div");
    pb1.className = "parallel-docs-page-break";
    pb1.setAttribute("data-parallel-docs-page-break", "true");
...
describe("maxRenderableParallelDocsContentBottomViewport", () => {
  beforeEach(() => {
    document.body.replaceChildren();
  });

  it("ignores tall page-break boxes when bounding parallel-docs above the next block anchor", () => {
    const scroll = document.createElement("div");
    scroll.style.position = "relative";

    const b1 = document.createElement("div");
    b1.id = "parallel-docs-block-x";

    const prose = pxBlock(document, 40, "Hello parallel-docs");
    const pb = document.createElement("div");
    pb.className = "parallel-docs-page-break";
    pb.setAttribute("data-parallel-docs-page-break", "true");
    ...
    const bottom = maxRenderableParallelDocsContentBottomViewport(scroll, b1, b2);
  ...
  it("includes parallel-docs that appears after an internal page break before the next block", () => {
    const scroll = document.createElement("div");
    scroll.style.position = "relative";
...
describe("parallelDocsGutterDocBandBottomViewport", () => {
  beforeEach(() => {
    document.body.replaceChildren();
  });
  ...
  it("stops the gutter doc band at the first page break before the next block (no false tie to interstitial prose)", () => {
    ...
    const bottom = parallelDocsGutterDocBandBottomViewport(scroll, b1, b2);
