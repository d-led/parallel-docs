...
describe("Mermaid runtime script injection", () => {
  ...
  it("inlines a self-contained vendored Mermaid build with no CDN references", () => {
    ...
    expect(html).toContain("globalThis.parallelDocsMermaid");
    expect(html).toContain(PARALLEL_DOCS_MERMAID_MODULE_READY_EVENT);
    expect(html).toContain("parallel-docs-mermaid-done");
  ...
  it("inlines a user-provided local Mermaid build instead of the vendored one", () => {
    const dir = mkdtempSync(join(tmpdir(), "parallel-docs-mermaid-"));
