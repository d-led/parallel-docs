 ...
 * Mirrors {@link bestParallelDocsAnchorLine0AtOrAboveY} in `code-browser-client.ts` for tests.
 *
 * Returns `null` when **no** anchor qualifies (e.g. probe falls in a tall gap before the first
 * anchor, or every anchor is below the probe). Callers must **not** treat that like parallel-docs
 * line 0 — that used to snap block-aware scroll sync back to the first block (“popping” the source).
 */
export function maxParallelDocsAnchorLine0AtOrAboveViewportY(
