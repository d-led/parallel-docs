...
export type BuildParallelDocsStaticOptions = {
  ...
  siteHubUrl?: string;
  /** Footer "Rendered with ParallelDocs" link plus semver and build time (`http`/`https` only). */
  toolHomeUrl?: string;
  /** When set, rewrites local and GitHub blob links in parallel-docs for static HTML output. */
  ...
  /**
   * `<meta name="generator">` value. When omitted, a default is built from `@parallel-docs/render` and
  ...
  /** Repo-relative companion Markdown path (with `staticSearchScope: "parallel-docs-and-paths"`). */
  ...
  /** GitHub blob URL for the companion parallel-docs Markdown file. */
  ...
  /** Relative URL to `parallel-docs-nav-search.json` for the documented-files tree. */
   ...
   * Set from CI (e.g. `PARALLEL_DOCS_PAGES_BUILD_SHA`); omit locally.
   */
...
function defaultGeneratorLabel(builtAt: Date): string {
  ...
  const name = pkg.name ?? "@parallel-docs/code-parallel-docs-static";
  const sv = pkg.version ?? "0.0.0";
  const iso = builtAt.toISOString();
  return `ParallelDocs @parallel-docs/render@${parallelDocsRenderVersion()}; ${name}@${sv}; builtAt=${iso}`;
}
...
async function readParallelDocsMarkdownForStaticBuild(
  mdPath: string,
  multi: BuildParallelDocsStaticOptions["multiAngleBrowsing"],
): Promise<string> {
...
async function copyCompanionStaticMirrors(
  copies: ParallelDocsStaticAssetCopy[] | undefined,
): Promise<void> {
...
export async function buildParallelDocsStatic(opts: BuildParallelDocsStaticOptions): Promise<void> {
  ...
  const parallelDocsMarkdown = await readParallelDocsMarkdownForStaticBuild(
    mdPath,
  ...
  const html = await renderCodeBrowserHtml({
    ...
    parallelDocsPathForSearch: opts.parallelDocsPathForSearch,
    ...
    parallelDocsOnGithubUrl: opts.parallelDocsOnGithubUrl,
    parallelDocsStaticBrowseUrl: opts.parallelDocsStaticBrowseUrl,
  ...
  await copyCompanionStaticMirrors(opts.parallelDocsOutputUrls?.companionStaticAssetCopies);
