# `blocks.md` — parallel-docs

The spec on the left is normative; this note is **why** the split between prose and metadata exists.

Humans own **parallel-docs** copy—headings, tone, diagrams. The tool owns **`index.json`**: ids, **`lines:`** / **`symbol:`** / **`marker:`** anchors, optional verification fields. That wall is what makes `validate` in CI meaningful without a parser rewriting your Markdown.
