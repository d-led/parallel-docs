# `block-stretch-buffer-sync.ts` — parallel-docs

<!-- #region parallelDocs:bsbs-dom -->

## GUI / DOM layer

This is where **synchronized abstract row slack** becomes **CSS geometry** in the static stretch table: per stretch `<tr>` the code measures intrinsic code and parallel-docs cell heights, builds `HeightAdjustable` rows, runs `BufferingFlowSynchronizer.synchronize`, then applies `bufferAbove` / `bufferBelow` as `padding-top` / `padding-bottom` on the paired `<td>` elements.
...
- Measure intrinsic heights in code vs parallel-docs cells.
...
Core algorithm stages and design rationale: [buffering-flow-synchronizer.ts parallel-docs](../../../core/src/buffering-flow-synchronizer.ts/main.md).

<!-- #endregion parallelDocs:bsbs-dom -->
